...
\begin{document}
  ...
  \section{Introduction}
    ...
    design problems grow to include tens of disciplines, it becomes increasingly difficult to construct the
  ...
  \section{Dependency Graph}\label{section:depgraph}
    ...
    one level, by excluding any disciplines that didn't directly contribute, it didn't handle a second level of sparsity
    ...
    related variables (i.e. as arrays) can be aggregated into a single node to represent the
    ...
    \subsection{Graph Traversal Determining Relevance}
        ...
        instead of seven if you did not reduce the graph. 
    ...
    \section{API for Specifying Discipline Derivatives}
        ...
        that declare derivatives, and a second that is optional for components that wish to take full
        ...
        The second method is \texttt{provideJ()}. This method serves two purposes. Firstly, it provides the
...
\end{minipage}
    ...
    The \textit{if} conditions in the sample implementation from Listing \ref{code:component_with_apply_deriv} provide the
    ...
    as it performs a mission over the course of one year. The full mission is represented by
    ...
    station, located in Ann Arbor, MI, over these six design points. There are seven design variables listed in Table~\ref{eqn:cadre_formulation}.
    ...
    \subsection{Optimization Results}
        ...
        data rates. Plotted geographically, this is due to the procession of
        ...
        station located at McMurdo Station, Antarctica, because its position closer to the South Pole 
        provided more opportunities to transmit data. The final optimal result gave 10340 GigaBytes
        transfered, or about a 2.6X improvement. Figure~\ref{fig:trajectories2} shows the ground trace 
        ...
        satellite. Notably there are now five different data transmission events compared to only three before. 
        ...
        \subsection{Performance Results}
            ...
            Reduction of the derivative dependency graph, as discussed in Section \ref{section:depgraph}, provides 
            a 30\% reduction in the computational cost to compute adjoint derivatives. Figure~\ref{fig:cadre-compute-cost}
            shows a log-log plot of computational cost for 31 adjoint solves (30 constraints + 1 objective) verses the 
            ...
            refining the time discretization for the integration of ODEs in the problem. While 
            ...
            \begin{table}
              ...
              \caption{Comparison between the full and reduced problem graphs for different sized small satellite design problems.}
                \begin{tabular}{c c c}
                    ...
                    \hline
                    1.08e6 & 8.8e5 &  18\%\\ \hline
                    2.15e6 & 1.76e6 & 18\%\\ \hline
                    4.30e6 & 3.52e6 & 18\%\\ \hline
                    8.60e6 & 7.05e6 & 18\%\\ \hline                    
                    1.72e7 & 1.41e7 & 18\%\\                    
                    \hline
                \end{tabular}   
            ...
            SNOPT itself. This problem requires a large number of minor iterations, where SNOPT solves a quadratic sub-problem. 
            The cost of solving these sub-problems is not directly affected by the cost of computing the derivatives and hence
            reduces overall computational savings. This characteristic will be highly problem dependent, and is not expected 
            to be so significant on all problems. 
  ...
  \section{Wind Turbine Design Problem}
    ...
    which shows the small satellite problem. The presence of these sub-assemblies is important to how the derivatives are computed. 
    ...
    \subsection{Performance Results}
        ...
        Figure~\ref{fig:wt-opt-progress} compares the performance between the two cases. For this problem, 
        finite difference performs reasonably well and achieves almost the same optimum design that the 
        analytic gradients case achieved. However, the analytic gradients allow the optimizer to converge 
        ...
        exceptions were all the analyses done with PBEAM. OpenMDAO automatically handled this situation by using finite 
        differences to approximate derivatives wherever they were missing. There are two different ways in which the framework 
        could perform the finite differencing and their application had significant performance implications. The first method 
        was to compute the full Jacobian via finite difference and provide it via the \textit{provideJ()} API. This solution was
        problematic when applied to PBEAM based analyses. The fundamental issue was that these analyses had hundreds 
        of inputs, compared to only 34 design variables at the model level. In order to construct the full Jacobian, hundreds of evaluations 
        were needed for each analysis, which resulted in a significant computational cost when compared to the 
        a full model finite difference. To resolve this issue, a directional finite difference was employed via the \textit{apply\_deriv()} API. 
        Using this approach, no explicit Jacobian is ever computed, and instead the linear operator is approximated directly. 
        Table~\ref{tab:wt-fd-speeds} shows that the directional derivative approach is much faster than the full Jacobian one. 

        \begin{table}
                \centering
                \caption{Compute time per major iteration for finite difference and the two methods of handling mixed derivatives.}
                \begin{tabular}{l c c c}
                    \hline
                                 & Pure FD & \textit{provideJ()} & \textit{apply\_deriv()}\\
                    \hline
                    Time (sec)  & 26.3 & 15.0 & 5.2 \\   % TODO: made up numbers
                    \hline
                \end{tabular}   
  ...
  \section{Conclusion}
      ...
      be relevant, and the Jacobians for any given discipline may be sparse as well. We implemented a graph-based 
      approach to identify the sparsity in any given formulation, with a graph structure designed to allow for 
      ...
      takes full advantage of all kinds of sparsity. The approach was applied to two different design problems, 
      each with unique characteristics that tested different aspects of the method. The small satellite design 
      problem had millions of variables and used all analytic adjoint derivatives.
      The results from that problem show the implementations scales well to large problems and demonstrated a significant 
      computational cost benefit from taking advantage of sparsity in the problem formulation. The wind turbine design 
      problem had a smaller design space, a more complex problem formulation, and used a combination of finite difference and analytic forward gradients. 
      Results from this problem showed a clear advantage in terms of computational efficiency from the application of 
      analytic derivatives, even to only some of the disciplines. In addition the results demonstrated the ability of 
      OpenMDAO to solve highly nested problems with coupled derivatives. 

      The combined results from the two design problems clearly demonstrate the value of this approach presented here. It enables 
